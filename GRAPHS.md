...
*End-to-end LangGraph routing pipeline from `START` to `END`. The `master_planner` is treated here as a single opaque block — its internal WebSupervisor dispatch, CyberAgent Zero-Trust pipeline, and all seven sub-agents are deliberately hidden and expanded in Graph 1B below. Every user message passes through `validator` unconditionally before reaching any downstream node.*
...
*Zooms into the `master_planner` dispatch cycle. `WebSupervisor` enforces the 6-step Zero-Trust pipeline through `CyberAgent`, then fans all seven sub-agents out in parallel via `asyncio.gather`. Steps 1–2 are outbound sanitisation before dispatch; Step 3 is the parallel agent wave itself; Steps 4–6 are inbound inspection after all agents return. A single injection-flagged TripContext field in Step 1 hard-blocks the entire dispatch and returns a `security_alert` without running any sub-agent.*
...
graph TD
    ...
    WS["WebSupervisor\nselects sub-agents · owns CyberAgent"]:::supervisorNode
    ...
    subgraph TIER1 ["Tier 1 · DB-backed Agents  (asyncio.gather — Step 3)"]
    ...
    subgraph TIER2 ["Tier 2 · Hierarchical Web Agents  (asyncio.gather — Step 3)"]
    ...
    MERGE["Merge raw_results\ncombined Dict[str, str] from all completed agents\n(failed agents do not block others)"]:::mergeNode
    ...
    STEP1 -- "clear" --> STEP2
    STEP2 -- "Step 3: asyncio.gather" --> TIER1 & TIER2
    TIER1 & TIER2 -- "raw results" --> MERGE
